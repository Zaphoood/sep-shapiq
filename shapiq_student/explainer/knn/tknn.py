...
class TKNNExplainer(KNNExplainerBase):
    ...
    def __init__(self, model: KNeighborsClassifier, tau: float, *args, **kwargs) -> None:
        super().__init__(model, *args, **kwargs)
        
        if not isinstance(tau, (int, float)):
            raise TypeError("`tau` must be a numeric type.")
        if tau <= 0:
...
def test_inheritance():
    ...
    print("=" * 40)
    print(f"TKNNExplainer isinstance of KNNExplainerBase: {isinstance(explainer, KNNExplainerBase)}")
    print(f"TKNNExplainer isinstance of Explainer: {isinstance(explainer, Explainer)}")
    print(f"KNNExplainerBase is subclass of Explainer: {issubclass(explainer.__class__.__bases__[0], Explainer)}")
    ...
    def tnn_shapley_single_original(x_train_few, y_train_few, x_test, y_test, tau=0, dis_metric='euclidean'):
        N = len(y_train_few)
        sv = np.zeros(N)
        C = max(y_train_few) + 1
